...
@section('content')
    <div class="x_panel">
        ...
        <div class="x_content">
            <br />
            <form method="POST" action="{{ route('reports.regiment.post') }}" id="demo-form2"
                class="form-horizontal form-label-left">
                @csrf
                <div id="" class="form-group">
                    <label class="control-label col-md-3 col-sm-3 col-xs-12"> الفوج
                        <span class="required">*</span>
                    </label>
                    <div class="col-md-6 col-sm-6 col-xs-12">
                        <select name="regiment" class="select_2 form-control">
                            <option value="all">كل الافواج</option>
                            @foreach ($regiments as $regiment)
                                <option value="{{ $regiment->id }}">{{ $regiment->id }}</option>
                            @endforeach
                        </select>
