...
<div class="x_panel">
    ...
    <div class="x_content">
        <br/>
        <form  method="POST" action="{{route('process.update',$process->id)}}" id="demo-form2"  class="form-horizontal form-label-left">
            @csrf
            {{ method_field('put') }}
                ...
                <div class="col-md-6 col-sm-6 col-xs-12">
                    <select name="agent_id" style="width: 100%" class="select_2 form-control">
                        @foreach ($agents as $item)
                            <option  {{$process->agent_id == $item->id ? 'selected' : ''}} value="{{$item->id}}">{{$item->name}}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="form-group">
                ...
                <div class="col-md-6 col-sm-6 col-xs-12">
                    <input type="text" value="{{$process->beneficiary}}" name="beneficiary" id="first-name" required="required"
            ...
            <div class="form-group">
                ...
                <div class="col-md-6 col-sm-6 col-xs-12">
                    <input type="text"  value="{{$process->beneficiary_phone}}" name="beneficiary_phone"  required="required"
                            class="form-control col-md-7 col-xs-12" value="">
                </div>
            </div>
        ...
        @livewire('edit-process',['process'=>$process, 'type'=>$type])
            ...
            <div class="form-group">
                ...
                <div class="col-md-6 col-sm-6 col-xs-12">
                    <textarea name="Statement" class="form-control" rows="3">@isset($process->daily[0]->Statement){{$process->daily[0]->Statement}}@endisset</textarea>
                </div>
            </div>
